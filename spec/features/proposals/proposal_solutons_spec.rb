...
#  it 'can create solutions in others debian proposal' do
#    @user = create(:default_user)
#    @user1 = create(:default_user)
#    @ability = Ability.new(@user)
#    @public_proposal = create(:public_proposal, quorum: BestQuorum.public.first, current_user_id: @user.id, proposal_type: ProposalType.find_by_name('DEBIAN'))
#
#    login_as @user1, scope: :user
#
#    visit edit_proposal_path(@public_proposal)
